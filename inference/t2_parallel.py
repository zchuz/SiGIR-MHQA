...
sys.path.append("/share/home/zchu/codes/train_2412")
...
from data_generation.utils import BM25_retrieval, dense_retrieval, completion_call_vllm, completion_call_vllm_with_dynamic_port, chat_completion_call_openai_msg
...
def guided_inference_v2(data):
    ...
    EXPLORATION_SIZE = 5 # Number of retrieval documents per retrieval step
    ...
    try:
        for round_index in range(9):
            ...
            documents = [BM25_retrieval(query, dataset=data["dataset"], topk=EXPLORATION_SIZE) for query in queries]
        ...
        for i in range(15):
            ...
            if i % 2 == 0:
                if "<paragraph>" in completion: # Do retrieval
                    ...
                    retrieval_contents = BM25_retrieval(query, dataset=data["dataset"], topk=5)
...
# Greedy inference for naive generator model after 12.27 (We slightly modified the instruction format.)
def vanilla_inference_v2(data):
    # if data["type"] not in ["comparison", "bridge_comparison"]:
    #     return
    ...
    try:
        for i in range(15):
            ...
            if i % 2 == 0:
                if "<paragraph>" in completion: # Do retrieval
                    query = extract_query(completion)
                    retrieval_content = BM25_retrieval(query, dataset=data["dataset"], topk=1)[0]
...
def inference_on_train_set(data):
    ...
    try:
        ...
        retrieval_content = BM25_retrieval(query, dataset=dataset, topk=3)[0]
