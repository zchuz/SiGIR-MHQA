...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, required=True, choices=["2wikimqa", "hotpotqa", "musique"], nargs="+", help="The dataset used for training.")
    ...
    args.output_dir = "./train/outputs/t2_naive/" + args.output_dir + "_" + get_random_suffix(n=3)        
...
def load_datas(args):
    datas = json.load(open(args.train_data_path))
    selected_datas = []
    for dataset in args.dataset:
        dataset_datas = [item for item in datas if item["meta"]["dataset"] == dataset]
        #random.shuffle(dataset_datas) Datas are already shuffled to help reproducibility
        selected_datas.extend(dataset_datas[:args.max_samples])
        logger.info(f"Loaded {len(dataset_datas[:args.max_samples])} datas from {dataset}.")
    
    logger.info(f"Loaded {len(selected_datas)} datas from {args.dataset}.")
...
def supervised_finetune(args):
    ...
    train_dataset = OursT2SFTDataset(datas=train_datas, tokenizer=tokenizer, args=args)
