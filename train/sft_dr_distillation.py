...
from train.utils import (
    load_model, 
    is_bf16_available, calculate_lr_scheduler, save_checkpoint, save_merged_checkpoint,
    get_logger, get_random_suffix, infer_model_name,
    ResponserSFTDataset, DecomposerSFTDataset
)
...
def parse_args():
    ...
    ## Datas Args
    parser.add_argument("--dataset", type=str, choices=["2wikimqa", "musique", "hotpotqa"], default="2wikimqa", nargs="+")
    parser.add_argument("--role", type=str, choices=["decomposer", "responser"])
    parser.add_argument("--teacher_model", type=str, choices=["gpt-4o-mini", "deepseek"])
    ...
    args.output_dir = "./train/outputs/dr_distillation/" + args.output_dir + "_" + get_random_suffix(n=3)        
...
def load_datas(args):
    ...
    if args.role == "decomposer":
        if "2wikimqa" in args.dataset:
            merged_datas.extend(json.load(open("./data_generation/dr_distillation/outputs/datasets/2wikimqa_dr_deepseek/decomposer.json")))
        elif "hotpotqa" in args.dataset:
            merged_datas.extend(json.load(open("./data_generation/dr_distillation/outputs/datasets/hotpotqa_dr_deepseek/decomposer.json")))
        elif "musique" in args.dataset:
            merged_datas.extend(json.load(open("./data_generation/dr_distillation/outputs/datasets/musique_dr_deepseek/decomposer.json")))
        else:
            raise ValueError(f"Dataset {args.dataset} is not supported currently.")
    elif args.role == "responser":
        if "2wikimqa" in args.dataset:
            merged_datas.extend(json.load(open("./data_generation/dr_distillation/outputs/datasets/2wikimqa_dr_deepseek/responser.json")))
        elif "hotpotqa" in args.dataset:
            merged_datas.extend(json.load(open("./data_generation/dr_distillation/outputs/datasets/hotpotqa_dr_deepseek/responser.json")))
        elif "musique" in args.dataset:
            merged_datas.extend(json.load(open("./data_generation/dr_distillation/outputs/datasets/musique_dr_deepseek/responser.json")))
        else:
            raise ValueError(f"Dataset {args.dataset} is not supported currently.")
    return merged_datas
...
def supervised_finetune(args):
    ...
    if args.accelerate_tracking:
        accelerator = Accelerator(
            ...
            mixed_precision=args.dtype if args.dtype != "auto" else (
                "bf16" if is_bf16_available() else "fp16"
            )
        ) 
    ...
    if args.role == "decomposer":
        train_dataset = DecomposerSFTDataset(datas=train_datas, tokenizer=tokenizer, args=args)
    elif args.role == "responser":
        train_dataset = ResponserSFTDataset(datas=train_datas, tokenizer=tokenizer, args=args)
    else:
        raise ValueError(f"{args.role} is not supported currently.")
    ...
    def collate_fn_train(examples):
        ...
        batch_encoding = tokenizer.pad(
            ...
            padding="longest",
            return_tensors="pt",
            max_length=1024 # This needs a hyperpameter to control
        )
